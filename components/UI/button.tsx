...
function PrimaryButton(props: Props): JSX.Element {
  const [styles, setStyles] = useState<ButtonStyleSheet>(AVALIABLE_STYLES.NORMAL);

  useEffect(() => setStyles(AVALIABLE_STYLES[props.buttonStyle]), []);

  return (
    <TouchableOpacity onPress={props.onPressEvent} touchSoundDisabled={true}>
      <View style={[styles.btn, props.additonalBtnStyles]}>
        <Text style={[styles.btnText, props.additonalBtnTextStyle]}>{props.buttonText}</Text>
      </View>
    </TouchableOpacity>
  );
}
